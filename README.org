...
:  Range (min … max):  15.460 ns … 24.585 ns  ┊ GC (min … max): 0.00% … 0.00%
:  Time  (median):     15.942 ns              ┊ GC (median):    0.00%
:  Time  (mean ± σ):   16.037 ns ±  0.486 ns  ┊ GC (mean ± σ):  0.00% ± 0.00%
: 
:         ▅██▇▆▄                                                 
:   ▁▁▁▂▄▇███████▆▄▄▃▃▃▂▂▁▂▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁ ▂
:   15.5 ns         Histogram: frequency by time        18.4 ns <
: 
...
Additionally, =default_buffer()= called on new tasks which are created inside the scope of =with_buffer= will create
buffers of the same size as the buffer you supply, i.e.

#+begin_src julia
let b = AllocBuffer(10)
    with_buffer(b) do
        @show sizeof(default_buffer())
        Threads.@spawn begin
            # Now that we're on a new thread, the default_buffer changes:
            @show default_buffer() != b
            # but it's still of the same length as b:
            @show sizeof(default_buffer()) == sizeof(b)
        end
    end
end;
#+end_src
...
#+RESULTS:
: sizeof(default_buffer()) = 10
: default_buffer() != b = true
: sizeof(default_buffer()) == sizeof(b) = true
